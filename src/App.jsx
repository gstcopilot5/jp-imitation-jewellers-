...
async function sbGetAnnouncements() {
  if (!sbReady()) return null;
  const r = await fetch(
    `${SUPABASE_URL}/rest/v1/announcements?select=*&active=eq.true&order=created_at.desc&limit=8`,
    { headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${SUPABASE_ANON_KEY}` } }
  );
  if (!r.ok) return null;
  return r.json();
}
async function sbSaveAnnouncement(token, ann) {
  const headers = {
    apikey: SUPABASE_ANON_KEY,
    Authorization: `Bearer ${token}`,
    "Content-Type": "application/json",
  ...
  const url = ann.id
    ? `${SUPABASE_URL}/rest/v1/announcements?id=eq.${ann.id}`
    : `${SUPABASE_URL}/rest/v1/announcements`;
  const r = await fetch(url, { method: ann.id ? "PATCH" : "POST", headers, body: JSON.stringify(ann) });
  if (!r.ok) throw new Error("save");
  return (await r.json())[0];
}
async function sbDeleteAnnouncement(token, id) {
...
/* analytics events */
function logEvent(type, page) {
  if (!sbReady()) return;
  try {
    fetch(`${SUPABASE_URL}/rest/v1/events`, {
      method: "POST",
      headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${SUPABASE_ANON_KEY}`, "Content-Type": "application/json" },
      body: JSON.stringify({ type, page: page || null }),
      keepalive: true,
    }).catch(() => {});
  } catch (e) {}
}
async function sbGetEvents(token) {
  if (!sbReady()) return [];
  const since = new Date(Date.now() - 7 * 864e5).toISOString();
  const r = await fetch(
    `${SUPABASE_URL}/rest/v1/events?select=type,page,created_at&created_at=gte.${since}&order=created_at.desc&limit=5000`,
    { headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${token}` } }
  );
  if (!r.ok) return [];
  return r.json();
...
create policy "admin offers" on offers
...
create policy "read active anns" on announcements
  for select to anon using (active = true);
create policy "admin anns" on announcements
  for all to authenticated using (true) with check (true);
...
const DEFAULT_ANNOUNCEMENTS = [
  { id: "a1", text: "Bridal season offer is live — up to 20% off select sets.", time: "Now" },
  { id: "a2", text: "New American-diamond arrivals in store this week.", time: "Today" },
  { id: "a3", text: "Open daily, 10:00 AM – 9:00 PM · Divanpara Main Road.", time: "Info" },
];
...
function NotifBell() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const a = await sbGetAnnouncements();
        if (on && a && a.length) {
          setItems(a.map((x) => ({ id: x.id, text: x.text, time: "New" })));
        }
      } catch (e) {}
    })();
    return () => { on = false; };
  ...
  return (
    <div className="tool-wrap">
      <button className="tool-btn" aria-label="Announcements" onClick={() => setOpen((o) => !o)}>
        <BellIcon />
        <span className="tool-dot" />
      </button>
      {open && (
        <>
          <div className="tool-overlay" onClick={() => setOpen(false)} />
          <div className="notif-pop">
            <p className="notif-head">Announcements</p>
            {items.map((n) => (
...
function BagButton() {
  ...
  return (
    <button className="tool-btn" aria-label="Enquiry bag" onClick={() => setBagOpen(true)}>
      <BagIcon />
      {bag.length > 0 && <span className="bag-badge">{bag.length}</span>}
    </button>
  );
}
...
/* ===== MOVING REVIEWS STRIP ===== */
const MARQUEE_REVIEWS = [
  "Awesome collection",
  "Huge collection & gorgeous styles",
  "Great quality at fair prices",
  "Found my full bridal set here",
  "The staff were so helpful",
  "Beautiful American-diamond pieces",
  "Loved the variety of necklaces",
  "Will visit again for the wedding",
];
function ReviewsMarquee() {
  const ref = useReveal();
  const row = [...MARQUEE_REVIEWS, ...MARQUEE_REVIEWS];
  return (
    <section className="rm-section" aria-label="Customer reviews" ref={ref}>
      <div className="section-head reveal" style={{ marginBottom: "2.4rem" }}>
        <span className="eyebrow">Loved In Rajkot</span>
        <Heading>4.8 ★ From 33+ Happy Customers</Heading>
      </div>
      <div className="rm-mask">
        <div className="rm-track">
          {row.map((q, i) => (
            <div className="rm-card" key={i}>
              <span className="stars">★★★★★</span>
              <p>“{q}”</p>
              <span className="rm-who">Google review</span>
            </div>
          ))}
        </div>
      </div>
...
function OfferEditor({ token, configured }) {
  ...
  useEffect(() => {
    if (!demo) (async () => { try { const o = await sbGetOffer(false); if (o) setOffer(o); } catch (e) {} })();
  }, [token]);
  ...
  const save = async () => {
    setBusy(true); setMsg("");
    try {
      if (demo) setMsg("Saved locally (demo). Connect Supabase to make it live.");
...
function AnnouncementsEditor({ token, configured }) {
  const demo = !configured || token === "demo";
  const [list, setList] = useState(demo ? DEFAULT_ANNOUNCEMENTS.map((a) => ({ id: a.id, text: a.text, active: true })) : []);
  const [text, setText] = useState("");
  const [busy, setBusy] = useState(false);
  const [msg, setMsg] = useState("");

  useEffect(() => {
    if (!demo) (async () => { try { const a = await sbAllAnnouncements(token); setList(a); } catch (e) {} })();
  }, [token]);
  ...
  const add = async () => {
    ...
    setBusy(true); setMsg("");
    try {
      if (demo) { setList((l) => [{ id: "tmp" + Date.now(), text, active: true }, ...l]); setMsg("Added (demo)."); }
      else { const a = await sbSaveAnnouncement(token, { text, active: true }); setList((l) => [a, ...l]); setMsg("Added & live."); }
      setText("");
    } catch { setMsg("Couldn't add — check permissions."); }
    finally { setBusy(false); }
  };
  const toggle = async (item) => {
    const upd = { ...item, active: !item.active };
    setList((l) => l.map((x) => (x.id === item.id ? upd : x)));
    if (!demo) { try { await sbSaveAnnouncement(token, upd); } catch (e) {} }
  };
  const del = async (item) => {
    setList((l) => l.filter((x) => x.id !== item.id));
    if (!demo) { try { await sbDeleteAnnouncement(token, item.id); } catch (e) {} }
  };

  return (
    <>
      <div className="field"><label>New announcement</label>
        <div className="row">
          <input className="inp" style={{ flex: 1 }} value={text} onChange={(e) => setText(e.target.value)} placeholder="e.g. Diwali offer — flat 15% off bridal sets" />
          <button className="btn btn-gold sm" onClick={add} disabled={busy}>Add</button>
        </div>
      </div>
      {msg && <p className="note ok">{msg}</p>}
      <div className="an-list">
        {list.length === 0 && <p className="note">No announcements yet.</p>}
        {list.map((a) => (
          <div className={`an-row ${a.active ? "" : "off"}`} key={a.id}>
            <span>{a.text}</span>
            <div className="an-actions">
              <button onClick={() => toggle(a)} title={a.active ? "Hide" : "Show"}>{a.active ? "Live" : "Hidden"}</button>
              <button onClick={() => del(a)} aria-label="Delete"><CloseIcon s={15} /></button>
            </div>
          </div>
        ))}
      </div>
...
function AnalyticsView({ token, configured }) {
  const demo = !configured || token === "demo";
  const [events, setEvents] = useState(demo ? [] : null);
  useEffect(() => {
    if (!demo) (async () => { try { const e = await sbGetEvents(token); setEvents(e); } catch (e) { setEvents([]); } })();
  }, [token]);

  if (events === null) return <p className="note">Loading analytics…</p>;

  const now = Date.now();
  const views = events.filter((e) => e.type === "pageview");
  const wa = events.filter((e) => e.type === "whatsapp_click").length;
  const enq = events.filter((e) => e.type === "enquiry_sent").length;
  const days = [...Array(7)].map((_, i) => {
    const d = new Date(now - (6 - i) * 864e5);
    const key = d.toISOString().slice(0, 10);
    return { label: d.toLocaleDateString(undefined, { weekday: "short" }), count: views.filter((e) => (e.created_at || "").slice(0, 10) === key).length };
  });
  const max = Math.max(1, ...days.map((d) => d.count));
  const pages = {};
  views.forEach((e) => { const k = e.page || "home"; pages[k] = (pages[k] || 0) + 1; });
  const topPages = Object.entries(pages).sort((a, b) => b[1] - a[1]).slice(0, 5);

  return (
    <>
      {demo && <p className="note">Demo mode — connect Supabase to see live numbers. Last 7 days shown.</p>}
      <div className="stat-grid">
        <div className="stat"><strong>{views.length}</strong><span>Page views</span></div>
        <div className="stat"><strong>{wa}</strong><span>WhatsApp clicks</span></div>
        <div className="stat"><strong>{enq}</strong><span>Enquiries sent</span></div>
      </div>
      <p className="an-sub">Views · last 7 days</p>
      <div className="bars">
        {days.map((d, i) => (
          <div className="bar-col" key={i}>
            <div className="bar" style={{ height: `${(d.count / max) * 100}%` }} title={`${d.count}`} />
            <span>{d.label}</span>
          </div>
        ))}
      </div>
      {topPages.length > 0 && (
        <>
          <p className="an-sub">Top pages</p>
          <div className="an-list">
            {topPages.map(([p, c]) => (
              <div className="an-row" key={p}><span>/{p === "home" ? "" : p}</span><div className="an-actions"><button>{c}</button></div></div>
            ))}
          </div>
        </>
      )}
      <p className="note" style={{ marginTop: "1.2rem" }}>
        Tip: also turn on Vercel Analytics (Vercel ▸ your project ▸ Analytics ▸ Enable) for accurate, privacy-friendly traffic with zero code.
      </p>
    </>
  );
}
    ...
    return (
      <div className="admin">
        <div className="admin-card">
          ...
          <h3 className="admin-h">Sign in to your dashboard</h3>
          {!configured && <p className="note">Supabase isn't connected yet — you can still explore in demo mode.</p>}
          <div className="field"><label>Email</label>
            <input className="inp" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@store.com" /></div>
          <div className="field"><label>Password</label>
            <input className="inp" type="password" value={pw} onChange={(e) => setPw(e.target.value)} placeholder="••••••••" /></div>
    ...
    <div className="admin">
      <div className="admin-card wide">
        <div className="admin-bar">
          <a href="#/" className="brand"><Diamond size={20} /> <span>J.P. <em>Admin</em></span></a>
          <button className="bag-clear" style={{ width: "auto" }} onClick={() => setToken(null)}>Sign out</button>
        </div>
...
const LEGAL = {
  privacy: {
    ...
    intro:
      "J.P. Imitation Jewellers respects your privacy. This page explains what limited information we collect through this website and how we use it.",
    sections: [
      { h: "Information we collect", p: "This website does not require you to create an account or make online payments. We only receive the details you choose to share with us directly — for example, the message and phone number you send when you contact us on WhatsApp or call the store." },
      { h: "How we use it", p: "We use your details only to respond to your enquiry, share collection information, and assist you with a purchase at our showroom. We do not sell or rent your information to third parties." },
      { h: "WhatsApp & calls", p: "When you contact us via WhatsApp, your message is handled under WhatsApp's own privacy terms. We retain enquiry details only as long as needed to serve you." },
      { h: "Cookies & analytics", p: "We may use privacy-friendly analytics to understand which pages are popular and how visitors use the site. This data is aggregated and does not identify you personally." },
      { h: "Your choices", p: "You may ask us to delete the enquiry details you have shared by contacting us at the number below." },
    ],
  },
  terms: {
    ...
    sections: [
      { h: "About this site", p: "This website showcases the imitation and fashion jewellery collections available at our Rajkot showroom. It is for information and enquiry purposes; purchases are completed in store or arranged directly with us." },
      { h: "Product images & descriptions", p: "We aim to present our designs accurately, but colours, finishes and stone placement may vary slightly between the images shown and the physical pieces, and across devices and screens." },
      { h: "Pricing & availability", p: "Any prices, offers or availability mentioned are indicative and may change without notice. Final price and availability are confirmed at the showroom." },
      { h: "Offers", p: "Promotional offers are valid for a limited time and on selected items only, and cannot be combined unless stated. We may withdraw or change an offer at any time." },
      { h: "Intellectual property", p: "The brand name, design and content of this website belong to J.P. Imitation Jewellers and may not be copied without permission." },
    ],
  },
  refund: {
    title: "Return & Refund Policy",
    intro: "We want you to be delighted with your purchase. This policy applies to items bought from our showroom.",
    sections: [
      { h: "Exchange window", p: "Unused items in original condition, with the bill, may be eligible for exchange within 7 days of purchase, subject to inspection at the store." },
      { h: "Non-returnable items", p: "For hygiene and quality reasons, certain items such as earrings, customised pieces and discounted/clearance items may not be eligible for return or exchange." },
      { h: "Refunds", p: "Where a refund is approved, it is processed to the original payment method within a reasonable period. Imitation jewellery is delicate; damage from misuse is not covered." },
      { h: "How to request", p: "Please bring the item and bill to the showroom, or contact us first on WhatsApp so we can guide you." },
    ],
  },
  shipping: {
    title: "Shipping & Delivery",
    intro: "Most of our business is at the showroom; delivery is arranged on request.",
    sections: [
      { h: "In-store collection", p: "You are welcome to view and collect your pieces directly at our Divanpara Main Road showroom in Rajkot." },
      { h: "Courier on request", p: "For customers who cannot visit in person, we may arrange courier delivery within India after confirming the order and details over WhatsApp or phone." },
      { h: "Timelines & charges", p: "Delivery timelines and any courier charges are confirmed at the time of order and depend on your location and the item." },
      { h: "Packaging", p: "All pieces are packed carefully for safe transit. Please check the package on delivery and report any concern to us promptly." },
    ],
  },
  disclaimer: {
    title: "Disclaimer",
    intro: "Please read this disclaimer carefully before using this website.",
    sections: [
      { h: "Imitation jewellery", p: "All products are imitation / fashion jewellery (also called artificial jewellery). They are not precious gold, silver or genuine diamonds unless explicitly stated in writing." },
      { h: "Accuracy", p: "We make every effort to keep information current and correct, but we do not warrant that all content is complete or error-free at all times." },
      { h: "External links", p: "This site may link to third-party services such as WhatsApp, Google Maps and social media. We are not responsible for the content or policies of those services." },
      { h: "Professional review", p: "These policies are provided in good faith. We recommend customers reach out to us directly for any clarification regarding a specific purchase." },
    ],
  },
};
...
function LegalPage({ pageKey }) {
  ...
  return (
    <section className="section legal" ref={ref}>
      ...
      <div className="legal-body reveal">
        ...
        <p className="legal-contact">
          Questions? Contact us at {PHONE_DISPLAY} or visit {ADDRESS}.
        </p>
      </div>
    </section>
  );
...
const ROUTE_META = {
  home: { title: "J.P. Imitation Jewellers | Premium Fashion Jewellery Store in Rajkot", desc: "Premium imitation jewellery, bridal sets, necklaces, earrings, bangles & American diamond collections at J.P. Imitation Jewellers, Rajkot." },
  collections: { title: "Collections | J.P. Imitation Jewellers, Rajkot", desc: "Explore bridal, American diamond, necklace, pendant, earring, bangle and fashion jewellery collections at J.P. Imitation Jewellers, Rajkot." },
  about: { title: "About | J.P. Imitation Jewellers, Rajkot", desc: "A trusted imitation jewellery destination on Divanpara Main Road, Rajkot — beautiful designs, quality and affordable luxury." },
  visit: { title: "Visit Us | J.P. Imitation Jewellers, Rajkot", desc: "Visit J.P. Imitation Jewellers on Divanpara Main Road, near Vishwakarma Temple, Rajkot. Directions, hours and contact." },
  privacy: { title: "Privacy Policy | J.P. Imitation Jewellers", desc: "How J.P. Imitation Jewellers handles the information you share." },
  terms: { title: "Terms & Conditions | J.P. Imitation Jewellers", desc: "Terms of use for the J.P. Imitation Jewellers website." },
  refund: { title: "Return & Refund Policy | J.P. Imitation Jewellers", desc: "Return, exchange and refund policy for J.P. Imitation Jewellers, Rajkot." },
  shipping: { title: "Shipping & Delivery | J.P. Imitation Jewellers", desc: "Shipping and delivery information for J.P. Imitation Jewellers." },
  disclaimer: { title: "Disclaimer | J.P. Imitation Jewellers", desc: "Website disclaimer for J.P. Imitation Jewellers." },
  admin: { title: "Admin | J.P. Imitation Jewellers", desc: "" },
};
...
function parseRoute(hash) {
  ...
  if (p.startsWith("refund")) return "refund";
  if (p.startsWith("shipping")) return "shipping";
  if (p.startsWith("disclaimer")) return "disclaimer";
  return "home";
}
function useRoute() {
...
function FabWhatsApp() {
  return (
    <a className="fab" href={WA()} target="_blank" rel="noreferrer" aria-label="WhatsApp" onClick={() => logEvent("whatsapp_click", "fab")}>
...
function PageView({ route }) {
  switch (route) {
    ...
    case "shipping":
    case "disclaimer":
      return <LegalPage pageKey={route} />;
    default:
      return (
        <>
          <Hero />
...
function BagDrawer() {
  const { bag, bagOpen, setBagOpen, removeFromBag, clearBag } = useBag();
  const send = () => {
    const lines = bag.map((x, i) => `${i + 1}. ${x.name}`).join("\n");
    logEvent("enquiry_sent", `${bag.length} items`);
    window.open(WA(`Hi J.P. Imitation Jewellers, I'm interested in these pieces:\n${lines}`), "_blank");
  };
  return (
    <>
      <div className={`drawer-backdrop ${bagOpen ? "show" : ""}`} onClick={() => setBagOpen(false)} />
      <aside className={`bag-drawer ${bagOpen ? "open" : ""}`} aria-hidden={!bagOpen}>
        <div className="bag-top">
          <h3>Your Enquiry Bag</h3>
          <button className="tool-btn" aria-label="Close" onClick={() => setBagOpen(false)}><CloseIcon /></button>
        </div>
        {bag.length === 0 ? (
          <div className="bag-empty">
            <Diamond size={30} />
            <p>Your bag is empty</p>
            <span>Add pieces you love, then send the whole list to us on WhatsApp.</span>
          </div>
        ) : (
          <>
            <div className="bag-list">
              {bag.map((x) => (
                <div key={x.name} className="bag-row">
                  <span>{x.name}</span>
                  <button aria-label={`Remove ${x.name}`} onClick={() => removeFromBag(x.name)}><CloseIcon s={16} /></button>
                </div>
              ))}
            </div>
            <div className="bag-foot">
              <button className="btn btn-gold" style={{ width: "100%" }} onClick={send}>
                <WaGlyph s={16} c="#1a0a0a" /> Send Enquiry on WhatsApp
              </button>
              <button className="bag-clear" onClick={clearBag}>Clear bag</button>
            </div>
          </>
        )}
      </aside>
...
function Toast() {
  ...
  return (
    <div className="toast" key={toast.id}>
      <Diamond size={16} /> <span>{toast.msg}</span>
    </div>
  );
}
...
export default function App() {
  ...
  // enquiry-bag store (session-based)
  ...
  const addToBag = useCallback((item) => {
    setBag((b) => (b.find((x) => x.name === item.name) ? b : [...b, item]));
    showToast(`${item.name} added to your bag`);
    setBagOpen(true);
  }, [showToast]);
  const removeFromBag = useCallback((name) => setBag((b) => b.filter((x) => x.name !== name)), []);
  const clearBag = useCallback(() => setBag([]), []);
  const store = { bag, bagOpen, setBagOpen, addToBag, removeFromBag, clearBag, toast };
  ...
  // SEO: per-route title + meta description, and pageview logging
  useEffect(() => {
    const m = ROUTE_META[shown] || ROUTE_META.home;
    ...
    logEvent("pageview", shown);
  }, [shown]);

  return (
    <BagContext.Provider value={store}>
...
.foot-grid{max-width:1280px;margin:0 auto;display:grid;grid-template-columns:1.6fr repeat(4,1fr);gap:2rem}
...
.tool-btn{position:relative;background:none;border:0;color:#e9e2d0;cursor:pointer;padding:6px;
  display:flex;align-items:center;justify-content:center;border-radius:10px;transition:.25s}
.tool-btn:hover{color:var(--gold);background:rgba(212,175,55,.08)}
.tool-dot{position:absolute;top:5px;right:6px;width:7px;height:7px;border-radius:50%;
  background:var(--gold);box-shadow:0 0 8px rgba(212,175,55,.9)}
.bag-badge{position:absolute;top:0;right:0;min-width:17px;height:17px;padding:0 4px;border-radius:9px;
  background:linear-gradient(135deg,#f0d98a,var(--gold));color:#1a0a0a;font-size:.66rem;font-weight:700;
  display:flex;align-items:center;justify-content:center;line-height:1}
.tool-overlay{position:fixed;inset:0;z-index:40}
.notif-pop{position:absolute;top:calc(100% + 12px);right:0;width:300px;max-width:84vw;z-index:55;
  background:rgba(16,8,10,.97);border:1px solid rgba(212,175,55,.25);border-radius:14px;padding:1rem;
  backdrop-filter:blur(14px);box-shadow:0 24px 60px rgba(0,0,0,.55);animation:pop .25s ease}
@keyframes pop{from{opacity:0;transform:translateY(-6px)}to{opacity:1;transform:none}}
.notif-head{font-size:.7rem;letter-spacing:.2em;text-transform:uppercase;color:var(--gold);margin-bottom:.8rem}
.notif-item{display:flex;gap:.6rem;padding:.6rem 0;border-top:1px solid rgba(212,175,55,.1)}
.notif-item:first-of-type{border-top:0}
.notif-item p{font-size:.85rem;color:#e9e2d0;margin:0}
.notif-item span{font-size:.68rem;color:var(--gold);opacity:.8}

/* ===== ENQUIRY BAG DRAWER ===== */
.drawer-backdrop{position:fixed;inset:0;background:rgba(0,0,0,.6);backdrop-filter:blur(2px);z-index:80;
  opacity:0;pointer-events:none;transition:opacity .35s}
.drawer-backdrop.show{opacity:1;pointer-events:auto}
.bag-drawer{position:fixed;top:0;right:0;height:100%;width:380px;max-width:88vw;z-index:90;
  background:linear-gradient(160deg,#160a0d,#0b0508);border-left:1px solid rgba(212,175,55,.22);
...
.bag-top{display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem}
.bag-top h3{font-size:1.3rem}
.bag-empty{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;gap:.6rem;color:var(--muted)}
.bag-empty p{font-family:'Bodoni Moda',serif;font-size:1.2rem;color:#fff}
.bag-empty span{font-size:.85rem;max-width:28ch}
.bag-list{flex:1;overflow-y:auto;margin:.4rem 0}
.bag-row{display:flex;align-items:center;justify-content:space-between;gap:.8rem;
  padding:.9rem .2rem;border-bottom:1px solid rgba(212,175,55,.12)}
.bag-row span{font-size:.95rem}
.bag-row button{background:none;border:0;color:var(--muted);cursor:pointer;padding:4px;border-radius:8px}
.bag-row button:hover{color:#fff;background:rgba(255,255,255,.06)}
.bag-foot{padding-top:1rem;border-top:1px solid rgba(212,175,55,.14)}
.bag-clear{display:block;width:100%;margin-top:.7rem;background:none;border:0;color:var(--muted);
  font-size:.82rem;letter-spacing:.05em;cursor:pointer}
.bag-clear:hover{color:var(--gold)}

/* ===== TOAST ===== */
.toast{position:fixed;left:50%;bottom:26px;transform:translateX(-50%);z-index:100;
  display:flex;align-items:center;gap:.5rem;padding:.8rem 1.3rem;border-radius:999px;
  background:rgba(16,8,10,.96);border:1px solid rgba(212,175,55,.4);color:#f3e6c4;
  font-size:.88rem;box-shadow:0 16px 44px rgba(0,0,0,.5);animation:toastin .35s ease}
@keyframes toastin{from{opacity:0;transform:translate(-50%,14px)}to{opacity:1;transform:translateX(-50%)}}

/* ===== COLLECTION ADD BUTTON ===== */
.col-cta-row{display:flex;align-items:center;gap:.9rem;flex-wrap:wrap}
.col-add{background:rgba(212,175,55,.12);border:1px solid rgba(212,175,55,.45);color:#f3e6c4;
  padding:.4rem .9rem;border-radius:999px;font-size:.76rem;letter-spacing:.04em;cursor:pointer;
  font-family:'Inter',sans-serif;transition:.25s}
.col-add:hover{background:var(--gold);color:#1a0a0a;border-color:var(--gold)}

/* ===== MOVING REVIEWS ===== */
.rm-section{padding:clamp(4rem,9vw,7rem) 0;max-width:100%;overflow:hidden}
.rm-section .section-head{padding:0 1.4rem}
.rm-mask{position:relative;width:100%;
  -webkit-mask:linear-gradient(90deg,transparent,#000 8%,#000 92%,transparent);
  mask:linear-gradient(90deg,transparent,#000 8%,#000 92%,transparent)}
.rm-track{display:flex;gap:1.2rem;width:max-content;animation:marquee 40s linear infinite}
.rm-track:hover{animation-play-state:paused}
@keyframes marquee{to{transform:translateX(-50%)}}
.rm-card{flex:0 0 auto;width:260px;padding:1.6rem 1.5rem;border-radius:16px;
  background:linear-gradient(160deg,rgba(255,255,255,.06),rgba(255,255,255,.02));
  border:1px solid rgba(212,175,55,.18)}
.rm-card .stars{color:var(--gold);letter-spacing:2px;font-size:.95rem}
.rm-card p{font-family:'Bodoni Moda',serif;font-style:italic;font-size:1.1rem;margin:.7rem 0 .9rem;line-height:1.35}
.rm-who{color:var(--muted);font-size:.72rem;letter-spacing:.08em}
@media (prefers-reduced-motion:reduce){.rm-track{animation:none}}

/* ===== ADMIN TABS / ANALYTICS ===== */
.admin-card.wide{max-width:680px}
.admin-bar{display:flex;align-items:center;justify-content:space-between;margin-bottom:1.2rem}
.admin-tabs{display:flex;gap:.4rem;margin-bottom:1.6rem;border-bottom:1px solid rgba(212,175,55,.16)}
.tab-btn{background:none;border:0;color:var(--muted);cursor:pointer;padding:.7rem 1rem;font-size:.9rem;
  font-family:'Inter',sans-serif;border-bottom:2px solid transparent;margin-bottom:-1px;transition:.25s}
.tab-btn:hover{color:#fff}
.tab-btn.active{color:var(--gold);border-bottom-color:var(--gold)}
.an-list{margin-top:1rem}
.an-row{display:flex;align-items:center;justify-content:space-between;gap:.8rem;padding:.8rem .2rem;
  border-bottom:1px solid rgba(212,175,55,.12);text-align:left}
.an-row.off{opacity:.5}
.an-row span{font-size:.9rem}
.an-actions{display:flex;align-items:center;gap:.4rem}
.an-actions button{background:rgba(212,175,55,.12);border:1px solid rgba(212,175,55,.3);color:#f3e6c4;
  border-radius:8px;padding:.3rem .6rem;font-size:.72rem;cursor:pointer;display:flex;align-items:center}
.an-actions button:hover{background:rgba(212,175,55,.2)}
.stat-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:.8rem;margin-bottom:1.4rem}
.stat{background:rgba(212,175,55,.06);border:1px solid rgba(212,175,55,.18);border-radius:12px;padding:1rem .6rem;text-align:center}
.stat strong{display:block;font-family:'Bodoni Moda',serif;font-size:1.8rem;color:var(--gold)}
.stat span{font-size:.7rem;color:var(--muted)}
.an-sub{font-size:.7rem;letter-spacing:.18em;text-transform:uppercase;color:var(--gold);margin:1.2rem 0 .8rem;text-align:left}
.bars{display:flex;align-items:flex-end;gap:.6rem;height:120px;padding:0 .2rem}
.bar-col{flex:1;display:flex;flex-direction:column;align-items:center;gap:.4rem;height:100%}
